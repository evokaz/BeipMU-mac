...
final class AutomationEditorWindowControllerTests: XCTestCase {
    ...
    func testTriggerSelectionPromptSavesDetailAndScopeValuesBeforeSwitching() throws {
        ...
        let content = try XCTUnwrap(controller.window?.contentView)
        let outline = try triggerOutline(in: content)
        let firstRow = try XCTUnwrap(row(titled: "First", in: outline))
        let secondRow = try XCTUnwrap(row(titled: "Second", in: outline))
        ...
        try textField(identifier: "triggerDescription", in: content).stringValue = "Edited First"
        try textField(identifier: "triggerScopeAfterCount", in: content).stringValue = "7"
        outline.selectRowIndexes(.init(integer: secondRow), byExtendingSelection: false)
        controller.outlineViewSelectionDidChange(.init(
            name: NSOutlineView.selectionDidChangeNotification,
            object: outline
    ...
    func testAliasSelectionPromptCancelRestoresDetailValues() throws {
        ...
        try textField(identifier: "aliasDescription", in: content).stringValue = "Edited First"
        ...
        XCTAssertEqual(try textField(identifier: "aliasDescription", in: content).stringValue, "Edited First")
    ...
    func testMacroSelectionPromptSavesDetailsBeforeSwitching() throws {
        ...
        try textField(identifier: "macroDescription", in: content).stringValue = "Edited First"
        ...
        XCTAssertEqual(try textField(identifier: "macroDescription", in: content).stringValue, "Second")
    }
